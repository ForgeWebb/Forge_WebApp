...
| `GEMINI_MODEL` | Optional, default `gemini-3.5-flash` (the oldest non-lite Flash still open, to keep the shared key's quota going as far as possible). Which model the AI features ask. A variable because Google closes models on its own schedule and a closed name breaks every AI feature at once — this way a swap is a variable change, not a deploy. A bare name (`gemini-3.5-flash-lite`) is fine; the `models/` prefix is added for you. **If AI suddenly stops working, check this first**: a closed model returns a 404 naming its replacement, and the log line (`Gemini request failed:`) quotes it verbatim. This is exactly what happened on 2026-09-23 — `gemini-2.5-flash` was closed to new keys, and because existing keys kept working it only broke for the newly-created shared key. |
